...
export default {
  ...
  methods: {
    queryMyLeave() {
      let result = this.api.getHistoryLeaveByUserId(this.userSelect.userId);
      result.then(re => {
        if(re.length > 0){
          this.dataHistory = re;
    ...
    setDataUserToComponent(){
      this.data_prop_profile_user.status = this.userSelect.statusWorking
      let data = {
          name: this.userSelect.name,
          email: this.userSelect.email,
          img: this.userSelect.image,
      }
  ...
  computed: {
    ...mapState({
      ...
      userSelect: state => state.leaveHistory.userId_select
    }),
    getListLeaveShow(){
